#!/bin/bash
DISK_DEVICE=/dev/vdb
CUR_DIR=/data/zhengyang/graphscope-flex/

export SF=0.1

export Scale_Factor=sf${SF}
export INPUT_OUTPUT_DIR=${CUR_DIR}/experiment_space/LDBC_SNB
export DB_ROOT_DIR=/nvme0n1/00new_db/${Scale_Factor}_db_BP_old
...
do
    ...
    rt_bench_thread -B ${memory_capacity} -l ${LOG_DIR}/graphscope_logs -g ${INPUT_OUTPUT_DIR}/configurations/graph_${SF}_bench.yaml -d ${DB_ROOT_DIR} -s ${thread_num} -w 0 -b 500000 -r ${QUERY_FILE} &>> ${LOG_DIR}/gs_log.log
    # gdb --args 
