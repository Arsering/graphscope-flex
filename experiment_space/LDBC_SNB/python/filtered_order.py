...
def breadth_first_degree_second_ordering(
    ...
    graph_replyOf_comment, graph_replyOf_post,comment_filter_set,post_filter_set):
    ...
    with tqdm(total=totalperson, desc="Processing ordering") as pbar:
        while len(person_list) < len(graph_knows_out):
            ...
            for person in Active:
                ...
                to_add_post = []
                to_add_comment = []
                # 处理 post 的 create 和 like
                for post in graph_post_creator.get(person, []):
                    if post not in added_posts and post not in post_filter_set:
                ...
                # 处理 likes_post
                to_add_post = []  # 清空 to_add_post
                for post in graph_likes_post.get(person, []):
                    if post not in added_posts and post not in post_filter_set:
                        to_add_post.append(post)
                        added_posts.add(post)
                to_add_post.sort(key=lambda x: degree_map_post[x], reverse=True)
                post_list.extend(to_add_post)  # 加入 post_list
                # 处理 comment 的 create 和 like
                for comment in graph_comment_creator.get(person, []):
                    if comment not in added_comments and comment not in comment_filter_set:
                ...
                # 处理 likes_comment
                to_add_comment = []  # 清空 to_add_comment
                for comment in graph_likes_comment.get(person, []):
                    if comment not in added_comments and comment not in comment_filter_set:
                        to_add_comment.append(comment)
                        added_comments.add(comment)
                to_add_comment.sort(key=lambda x: degree_map_comment[x], reverse=True)
                comment_list.extend(to_add_comment)  # 加入 comment_list
...
def reorder_and_write_csv_comment(original_csv, new_csv, ordered_ids):
    # 读取原始 CSV 文件的数据
    with open(original_csv, 'r') as f:
        reader = csv.reader(f, delimiter='|')
        header = next(reader)  # 保存表头
    ...
    # 找到未在ordered_ids中的ID，追加到列表末尾
    remaining_data = [row for pid, row in tqdm(data.items(), desc="Finding remaining data", unit="id") if pid not in inset]
    # 按照 creationDate 的递增顺序排序 remaining_data
    def parse_date(date_str):
        try:
            ...
            try:
                ...
                return None
    remaining_data.sort(key=lambda row: parse_date(row[1]))  # 假设 creationDate 是第2列
...
def reorder_and_write_csv_post(original_csv, new_csv, ordered_ids):
    # 读取原始 CSV 文件的数据
    with open(original_csv, 'r') as f:
        reader = csv.reader(f, delimiter='|')
        header = next(reader)  # 保存表头
    ...
    def parse_date(date_str):
        try:
            ...
            try:
                ...
                print(f"无法解析日期: {date_str}")
                return None
    remaining_data.sort(key=lambda row: parse_date(row[2]))  # 假设 creationDate 是第2列
...
def set_filter(filter_ratio):
    # 存储id和creationDate的字典
    comment_filtered_set=set()
    post_filtered_set=set()
    # 打开并读取CSV文件
        ...
        for row in reader:
            ...
            try:
                ...
                try:
                    ...
                    continue
            # 将id和转换后的日期存入字典
            id_to_creation_date[record_id] = parsed_date
        sorted_id_to_creation_date = sorted(id_to_creation_date.items(), key=lambda item: item[1])
        filter_point=int(filter_ratio*len(sorted_id_to_creation_date))
        for i in range(filter_point,len(sorted_id_to_creation_date)):
            comment_filtered_set.add(sorted_id_to_creation_date[i][0])
    # 打开并读取CSV文件
    with open(post_csv, mode='r', encoding='utf-8') as file:
        ...
        filter_point=int(filter_ratio*len(sorted_id_to_creation_date))
        for i in range(filter_point,len(sorted_id_to_creation_date)):
            post_filtered_set.add(sorted_id_to_creation_date[i][0])
    return comment_filtered_set,post_filtered_set
...
comment_filter_set, post_filter_set=set_filter(0.5)
print(len(comment_filter_set))
...
person_list, post_list, comment_list = breadth_first_degree_second_ordering(
    ...
    comment_filter_set,post_filter_set
)

# 根据person_list, comment_list, post_list重新排序文件并输出
reorder_and_write_csv(person_csv, new_person_csv, person_list)
reorder_and_write_csv_comment(comment_csv, new_comment_csv, comment_list)
reorder_and_write_csv_post(post_csv, new_post_csv, post_list)
