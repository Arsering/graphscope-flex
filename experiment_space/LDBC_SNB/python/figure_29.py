...
def plot_figure_2(index, legends, xticks):
    ...
    for idx in range(breakdown.shape[0]):
        print(idx)
        ax1.bar(np.array(index[0])+index[1][idx], np.array(breakdown[idx, :]), width=bar_width, color='white', edgecolor=edgecolors[idx], hatch=hatchs[idx], linewidth=2, zorder=100)
