...
def data_analysis_inner(file_path, datas, count):
    ...
    while line:
        ...
        if int(logs[2]) != 7:
            line = f.readline()
            continue
        datas[count, 0] = int(logs[9])
        if int(logs[0]) > time_max:
            time_max = int(logs[0])
        if int(logs[0]) < time_min:
            time_min = int(logs[0])
        count += 1
        if count == 3000000:
            break
    ...
    print((time_max-time_min)/1000/60)
    return count
...
if __name__ == "__main__":
    ...
    data_dir = '/nvme0n1/lgraph_db/sf30/social_network/updateStream_0_9_forum.csv'
    # datas, count = data_analysis(data_dir)
    
    datas = np.zeros([3000000, 1])
    count = 0
    count = data_analysis_inner(data_dir, datas, count)

    datas = datas[:count]
    print(count)
    # 使用np.unique获取唯一值及其出现次数
    unique_elements, counts = np.unique(np.int64(datas), return_counts=True)

    # 打印结果
    count_all = []
    for unique_element, count in zip(unique_elements, counts):
        # print(f"Value {unique_element} appears {count} times.")
        count_all.append(count)
    degree_counter = count_all
