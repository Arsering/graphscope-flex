...
seastar::future<query_result> executor_ref::run_hqps_adhoc_query(query_param &&param) {
	addr.set_method_actor_tid(0);
	return hiactor::actor_client::request<query_result, server::query_param>(addr, k_executor_run_hqps_adhoc_query, std::forward<server::query_param>(param));
}

seastar::future<query_result> executor_ref::run_hqps_procedure_query(query_param &&param) {
	addr.set_method_actor_tid(0);
	return hiactor::actor_client::request<query_result, server::query_param>(addr, k_executor_run_hqps_procedure_query, std::forward<server::query_param>(param));
}
...
seastar::future<hiactor::stop_reaction> executor::do_work(hiactor::actor_message* msg) {
	switch (msg->hdr.behavior_tid) {
		...
		case k_executor_run_hqps_adhoc_query: {
			...
			return apply_run_hqps_adhoc_query(msg, this).then_wrapped([msg] (seastar::future<query_result> fut) {
				if (__builtin_expect(fut.failed(), false)) {
					auto* ex_msg = hiactor::make_response_message(
						msg->hdr.src_shard_id,
						hiactor::simple_string::from_exception(fut.get_exception()),
		...
		case k_executor_run_hqps_procedure_query: {
			...
			return apply_run_hqps_procedure_query(msg, this).then_wrapped([msg] (seastar::future<query_result> fut) {
				if (__builtin_expect(fut.failed(), false)) {
					auto* ex_msg = hiactor::make_response_message(
						msg->hdr.src_shard_id,
						hiactor::simple_string::from_exception(fut.get_exception()),
