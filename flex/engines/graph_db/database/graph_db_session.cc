...
InsertTransaction GraphDBSession::GetInsertTransaction() {
  ...
  return InsertTransaction(db_.graph_, alloc_, logger_, db_.version_manager_, ts);
}

SingleVertexInsertTransaction GraphDBSession::GetSingleVertexInsertTransaction() {
  uint32_t ts = db_.version_manager_.acquire_insert_timestamp();
  return SingleVertexInsertTransaction(db_.graph_, alloc_, logger_, db_.version_manager_, ts);
}

SingleEdgeInsertTransaction GraphDBSession::GetSingleEdgeInsertTransaction() {
  uint32_t ts = db_.version_manager_.acquire_insert_timestamp();
  return SingleEdgeInsertTransaction(db_.graph_, alloc_, logger_, db_.version_manager_, ts);
}

UpdateTransaction GraphDBSession::GetUpdateTransaction() {
  uint32_t ts = db_.version_manager_.acquire_update_timestamp();
  return UpdateTransaction(db_.graph_, alloc_, logger_, db_.version_manager_, ts);
}
...
std::vector<char> GraphDBSession::Eval(const std::string& input) {
  ...
  if (likely(apps_[type] != nullptr)) {
    if (!apps_[type]->Query(decoder, encoder)) {
      LOG(ERROR) << "[Query-" + std::to_string((int) type) << "][Thread-" << thread_id_ << "] failed...";
      result_buffer.clear();
    }
  } else {
    app_wrappers_[type] = db_.CreateApp(type, thread_id_);
    if (app_wrappers_[type].app() == NULL) {
      LOG(ERROR) << "[Query-" + std::to_string((int) type) << "] is not registered...";
    } else {
...
void GraphDBSession::RegisterApp(const std::string& path, uint8_t index) {
  db_.RegisterApp(path, index);
}

void GraphDBSession::GetAppInfo(Encoder& result) {
  db_.GetAppInfo(result);
}

int GraphDBSession::SessionId() const {
  return thread_id_;
}
