...
struct SessionLocalContext {
  SessionLocalContext(GraphDB& db, int thread_id) : session(db, allocator, logger, thread_id) {}
  ~SessionLocalContext() {
    logger.close();
  }
  ...
  WalWriter logger;
  char _padding1[4096 - sizeof(WalWriter) - sizeof(ArenaAllocator) - sizeof(_padding0)];
...
void GraphDB::Init(const Schema& schema,
  ...
  std::vector<std::string> plugins = plugins_;
  if (!std::filesystem::exists(serial_path)) {
    ...
    if (!plugins.empty()) {
      grape::InArchive arc;
      arc << plugins;
      std::string plugins_list_file_path = data_dir_path / "plugins.info";
      FILE* fout = fopen(plugins_list_file_path.c_str(), "wb");
      size_t arc_size = arc.GetSize();
      CHECK_EQ(fwrite(&arc_size, sizeof(size_t), 1, fout), 1);
      CHECK_EQ(fwrite(arc.GetBuffer(), sizeof(char), arc_size, fout), arc_size);
      fflush(fout);
      fclose(fout);
    }
  } else {
    ...
    std::string plugins_list_file_path = data_dir_path / "plugins.info";
    if (std::filesystem::exists(plugins_list_file_path)) {
      FILE* fin = fopen(plugins_list_file_path.c_str(), "r");
      size_t arc_size;
      CHECK_EQ(fread(&arc_size, sizeof(size_t), 1, fin), 1);
      grape::OutArchive arc;
      arc.Allocate(arc_size);
      CHECK_EQ(fread(arc.GetBuffer(), sizeof(char), arc_size, fin), arc_size);
      fclose(fin);
      arc >> plugins;
    }
  }
...
SingleVertexInsertTransaction GraphDB::GetSingleVertexInsertTransaction(int thread_id) {
  return contexts_[thread_id].session.GetSingleVertexInsertTransaction();
}

SingleEdgeInsertTransaction GraphDB::GetSingleEdgeInsertTransaction(int thread_id) {
...
AppWrapper GraphDB::CreateApp(uint8_t app_type, int thread_id) {
  std::lock_guard<std::mutex> guard(apps_lock_);
...
static void IngestWalRange(
    SessionLocalContext* contexts,
    MutablePropertyFragment& graph, const WalsParser& parser, uint32_t from,
  ...
  for (int i = 0; i < thread_num; ++i) {
    threads[i] = std::thread([&](int tid) {
      ...
      while (true) {
        ...
        InsertTransaction::IngestWal(graph, got_ts, unit.ptr, unit.size, alloc);
        if (got_ts % 1000000 == 0) {
