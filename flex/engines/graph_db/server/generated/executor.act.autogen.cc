...
enum : uint8_t {
	k_executor_run_query = 0,
};

executor_ref::executor_ref() : ::hiactor::reference_base() { actor_type = 0; }

seastar::future<query_result> executor_ref::run_query(server::query_param &&param) {
	addr.set_method_actor_tid(0);
	return hiactor::actor_client::request<server::query_result, server::query_param>(addr, k_executor_run_query, std::forward<server::query_param>(param));
}

seastar::future<hiactor::stop_reaction> executor::do_work(hiactor::actor_message* msg) {
	switch (msg->hdr.behavior_tid) {
		case k_executor_run_query: {
			static auto apply_run_query = [] (hiactor::actor_message *a_msg, executor *self) {
				if (!a_msg->hdr.from_network) {
					return self->run_query(std::move(reinterpret_cast<
					...
					return self->run_query(server::query_param::load_from(ori_msg->data));
				}
			};
			return apply_run_query(msg, this).then_wrapped([msg] (seastar::future<server::query_result> fut) {
