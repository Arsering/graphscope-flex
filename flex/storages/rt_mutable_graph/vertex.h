...
class Vertex {
 public:
  struct ColumnConfiguration {
    ColumnConfiguration(size_t property_id_in, size_t column_family_id_in,
                        gs::PropertyType column_type_in,
                        PropertyType edge_type_in)
  ...
  struct ColumnToColumnFamily {
    ColumnToColumnFamily(size_t property_id, gs::PropertyType column_type,
            ...
            if (column_configuration.column_type ==
              ...
              mmap_array_ptr->open(
                  db_dir_path_ + "/" + vertex_name_ + "/column_family_" +
                      ...
                      std::to_string(edge_list_column_id) + ".edgelist",
                  false);
  ...
  void Resize(size_t new_capacity_in_row) {
    for (auto& data_per_column_family : datas_of_all_column_family_) {
      ...
      for (auto& csr : data_per_column_family.csr) {
        csr.second->resize(new_capacity_in_row * 128);
      }
    }
