...
using WeightedGraph = immutable_graph::ImmutableGraph<int64_t, uint32_t, grape::EmptyType, double, grape::LoadStrategy::kOnlyOut, grape::GlobalVertexMap<int64_t, uint32_t, grape::SegmentedPartitioner<int64_t>>>;
using NonWeightedGraph = immutable_graph::ImmutableGraph<int64_t, uint32_t, grape::EmptyType, grape::EmptyType, grape::LoadStrategy::kOnlyOut, grape::GlobalVertexMap<int64_t, uint32_t, grape::SegmentedPartitioner<int64_t>>>;
...
void DoQuery(std::shared_ptr<FRAG_T> fragment, std::shared_ptr<APP_T> app,
             const grape::CommSpec& comm_spec, const std::string& out_prefix, Args... args) {
...
int main(int argc, char** argv) {
  ...
  if (name == "sssp") {
    auto fragment = grape::LoadGraph<WeightedGraph>(FLAGS_efile, FLAGS_vfile, comm_spec, graph_spec);
    using AppType = bsp::SSSPApp<WeightedGraph>;
    auto app = std::make_shared<AppType>();

    DoQuery(fragment, app, comm_spec, out_prefix, FLAGS_sssp_source);
  } else {
    auto fragment = grape::LoadGraph<NonWeightedGraph>(FLAGS_efile, FLAGS_vfile, comm_spec, graph_spec);
    if (name == "bfs") {
